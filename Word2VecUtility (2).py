...
class Word2VecUtility():
    ...
    def getCleanReviews(self,reviews):
        clean_reviews = []
        # review 为str reviews为dataframe 里面的项为str类型 处理的是中文patent 数据集
        if reviews.get("description",None)!=None:
            for review in reviews["description"]:
                sentInreview=self.tokenreviewtoSentence(review)
                tokenReview=[]
                #将每个句子 处理后 拼接到一个list
                for sent in sentInreview:
                    tokenReview+=Word2VecUtility.review_to_wordlist(sent, remove_stopwords=True ,low_freq_filter=True)
        ...
        # review 处理的是 Stanford sst 数据集
            ...
            for review in reviews["content"]:
                ...
                #将每个句子 处理后 拼接到一个list
                for sent in sentInreview:
                    tokenReview+=Word2VecUtility.review_to_wordlist_en(sent, remove_stopwords=True ,low_freq_filter=True)
                # 将这个review  of list 添加到clean_review (list of list [[],[]])  
                clean_reviews.append(tokenReview)
    ...
    def getCleanReviewsStr(self,reviews):
        clean_reviews = []
        #  review 为str reviews为dataframe 里面的项为str类型 处理的是中文patent 数据集
        if reviews.get("description",None)!=None:
            for review in reviews["description"]:
